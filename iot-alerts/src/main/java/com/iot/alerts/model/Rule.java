 ...
 *                        rules)
 * @param cooldownSeconds Cooldown time between repeated alerts for the same
 *                        device/rule pair
 */
public record Rule(
        String id,
    ...
    public static Rule instant(String id, String name, Severity severity,
            Predicate<DeviceData> condition,
            Function<DeviceData, Object> valueExtractor,
            Object threshold, int cooldownSeconds) {
        return new Rule(id, name, RuleType.INSTANT, severity, condition, valueExtractor, threshold, 1, cooldownSeconds);
    }
    ...
    public static Rule duration(String id, String name, Severity severity,
            Predicate<DeviceData> condition,
            Function<DeviceData, Object> valueExtractor,
            Object threshold, int requiredPackets, int cooldownSeconds) {
        return new Rule(id, name, RuleType.DURATION, severity, condition, valueExtractor, threshold, requiredPackets, cooldownSeconds);
    }
