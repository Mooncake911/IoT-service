...
@DisplayName("Analytics RabbitMQConfig Tests")
class RabbitMQConfigTest {

    private RabbitMQConfig config() {
        RabbitMQConfig config = new RabbitMQConfig();
        ReflectionTestUtils.setField(config, "dataExchangeName", "iot.data");
        ReflectionTestUtils.setField(config, "analyticsQueueName", "iot.analytics");
        ReflectionTestUtils.setField(config, "analyticsQueueTtl", 60000);
        return config;
    }
    ...
    void infrastructureBeans_shouldUseProperties() {
        ...
        Queue queue = config.analyticsQueue();
        assertThat(queue.getName()).isEqualTo("iot.analytics");

        Binding binding = config.analyticsBinding(queue, exchange);
